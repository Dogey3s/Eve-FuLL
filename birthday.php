...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = "INSERT INTO birthday_req (name, event_date, event_location, cake, num_guests, food_type, dress_type, flower_decoration, photo_video, comment) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    // Use prepared statement to prevent SQL injection
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssssss", $userName, $eventDate, $eventLocation, $cake, $numGuests, $foodType, $dressType, $flowerDecoration, $photovideo, $comment);
    $stmt->execute();
    $stmt->close();
