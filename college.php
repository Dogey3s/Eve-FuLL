...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = "INSERT INTO college_req (name, event_date, event_location, num_guests, stage_deco, arr_deco, food_type, comment) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

    // Use prepared statement to prevent SQL injection
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssss", $userName, $eventDate, $eventLocation, $numGuests, $stagedeco, $arrdeco, $foodType, $comment);
    $stmt->execute();
    $stmt->close();
