...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = "INSERT INTO user_req (name, event_date, event_location, num_guests, food_type, dress_type, flower_decoration, comment, photo_video) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

    // Use prepared statement to prevent SQL injection
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssssss", $userName, $eventDate, $eventLocation, $numGuests, $foodType, $dressType, $flowerDecoration, $comment, $photovideo);
    $stmt->execute();
    $stmt->close();
